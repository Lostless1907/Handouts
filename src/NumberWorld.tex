...
The real numbers $\R$ resolve division and eliminate gaps, but still fail at a fundamental level:
...
Thus $\R$ is not sufficient to solve all polynomial equations.

We now ask:

\textit{Can we extend $\R$ in the smallest possible way so that this equation has a solution?}

\begin{theorybox}

\textbf{Step 1: Forcing a Solution}

Introduce a new element $i$ such that:
\[
i^2 = -1
\]

This is the minimal extension required to solve:
\[
x^2 + 1 = 0
\]

\textbf{Step 2: What Should Numbers Look Like Now?}

If $i$ exists, then expressions like $a + bi$ must be included.

To remain closed under addition and multiplication, we consider all such expressions:
\[
...
A complex number is an ordered pair $(x,y)$ written as $x + iy$.

\textbf{Step 3: Defining Addition}

To extend real addition consistently:
\[
(a,b) + (c,d) = (a+c, b+d)
\]
...
\textbf{Step 4: Defining Multiplication}
...
This is the only rule that preserves the required properties.

\textbf{Step 5: Algebraic Structure}
...
\textbf{Step 6: Embedding $\R$}

Every real number appears inside $\C$:
\[
x = x + 0i
\]
...
\textbf{Step 7: Identity Elements}
...
These extract the two coordinates of the number.
...
Then:
\[
z\bar{z} = (x+iy)(x-iy) = x^2 + y^2 \in \R
\]

This converts a complex number into a real quantity.

\subsection*{Numbers as Points}

The expression $x + iy$ behaves like a point $(x,y)$.

Thus $\C$ can be viewed as a plane.

\subsection*{Trigonometry and Direction}
...
Each angle $\theta$ represents a direction.
...
If $z \neq 0$, there exists $\theta$ such that:
\[
z = |z|(\cos\theta + i\sin\theta)
\]

Each number has magnitude and direction.
...
Multiplication scales lengths and adds angles.

\item \textbf{No Order Relation}

There is no consistent way to define $<$ or $>$ on $\C$.

\item \textbf{Algebraic Closure}

Every polynomial equation has a solution in $\C$.
